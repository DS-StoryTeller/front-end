...
const Signin = ({navigation}) => {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      const response = await fetch('http://172.30.1.6:8080/register', {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        Alert.alert('오류', text || '회원가입에 실패했습니다');
      }
    } catch (error) {
      console.error(error);
  ...
  const handleEmailVerificationRequest = async () => {
    try {
      const response = await fetch(
        'http://172.30.1.6:8080/emails/verification-requests',
        {
          method: 'POST',
          headers: {
  ...
  const handleEmailVerificationCheck = async () => {
    try {
      const response = await fetch(
        'http://172.30.1.6:8080/emails/verifications',
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleUsernameVerification = async () => {
    try {
      const response = await fetch(
        'http://172.30.1.6:8080/username/verifications',
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
